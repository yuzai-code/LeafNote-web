<template>
    <div class="folder-tree">
        <!-- 当前目录项 -->
        <div class="flex items-center gap-2 p-2 hover:bg-base-200 rounded-lg cursor-pointer" :class="{
            'pl-[calc(12px*var(--depth,1))]': folder.path.split('/').length - 1,
        }" :style="{ '--depth': folder.path.split('/').length - 1 }" @click="toggleFolder">
            <div class="flex items-center gap-2 flex-1">
                <!-- 展开/折叠图标 -->
                <svg v-if="hasChildren" class="w-3.5 h-3.5 transition-transform"
                    :class="{ 'rotate-90': folder.expanded }" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                ...
                <!-- 目录名称/重命名输入框 -->
                <div class="flex-1">
                    <input v-if="folder.isEditing" :ref="(el) => setInputRef(el, folder.id)"
                        v-model="folder.editingName" class="input input-sm input-bordered w-full" @blur="handleRename"
                        @keyup.enter="handleRename" @keyup.esc="cancelRename" @click.stop />
                    <span v-else class="text-sm">{{ folder.name }}</span>
                </div>
            </div>
            <div @click.stop>
                <FolderItem @create-note="handleCreateNote" @create-folder="handleCreateFolder" @rename="startRename"
                    @delete="handleDelete" />
            </div>
        </div>
        ...
        <div v-if="folder.expanded" class="mt-1">
            ...
            <template v-if="folder.children?.length">
                <TreeFolder v-for="child in folder.children" :key="child.id" :folder="child"
            ...
            <template v-if="folder.notes?.length">
                <div v-for="note in folder.notes" :key="note.id" class="cursor-pointer" @click="handleNoteClick(note)">
                    <div class="flex items-center gap-2 p-2 hover:bg-base-200 rounded-lg" :class="{
